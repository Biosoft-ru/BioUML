...
{
    private static final String TIME_INCREMENT_VARIABLE = "__TIME__INCREMENT__";
    ...
    {
        return agentModel != null ? StreamEx.of( agentModel.getAgents() ).select( PlotAgent.class ).toArray( PlotAgent[]::new )
                : new PlotAgent[0];
    }
    ...
    public AgentBasedModel createModel() throws Exception
    {
        ...
        for( AgentSimulationEngineWrapper engine : newEngines )
        {
            ...
            setRecalculateStep((ModelAgent)agent, engine);
    ...
    private void setRecalculateStep(ModelAgent agent, SimulationEngine engine)
    {
        EModel emodel = engine.getExecutableModel();
        Variable stepVariable = emodel.getVariable( TIME_INCREMENT_VARIABLE );
        if( stepVariable == null )
            return;
        int index = engine.getVarPathIndexMapping().get( TIME_INCREMENT_VARIABLE );
        agent.setRecalculateStep( index );
    }
    ...
    private void addConnection(Connection connection) throws Exception
    {
        ...
        else if( connection instanceof UndirectedConnection )
        {
            ...
            agentModel.addUndirectedLink( sender, inputVarName, receiver, outputVarName, type.equals( MainVariableType.INPUT ),
                    conversionFactor );
        }
    }
    ...
    private Diagram generateMainAgent(Diagram diagram, boolean processBuses) throws Exception
    {
        ...
        for( DiagramElement de : innerDiagram.recursiveStream().toSet() )
        {
            if( de instanceof DiagramContainer || Util.isPlot( de ) || Util.isSwitch( de ) || Util.isAverager( de ) || Util.isConstant( de )
                    || ( de.getKernel() != null
                            && ( de.getKernel().getType().equals( "ScriptAgent" ) || de.getKernel().getType().equals( "PythonAgent" ) ) )
                    || Util.isConnection( de ) )
    ...
    private Diagram flatSimilarSubdiagrams(Diagram diagram, List<AgentSimulationEngineWrapper> engines) throws Exception
    {
        ...
        for( EnginesCategory engineGroup : enginesCategories )
        {
            ...
            Set<SubDiagram> subDiagrams = StreamEx.of( localEngines ).map( engine -> engine.de )
                    .map( de -> diagram.findNode( de.getName() ) ).select( SubDiagram.class ).toSet();

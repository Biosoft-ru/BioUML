...
public class KaraaslanNewModel extends JavaBaseModel
{
    ...
    private void __internalVarInitFunc_0(double time, double[] x)
    {
        ...
        R_ba = K_bar / x[6];
        R_tp = R_ba * eps_aum + R_bv;

        R_vr = ( 8 * R_bv + R_ba * eps_aum ) / 31;
...
//        result[2] = + ( N_adhs - x[2] ) / T_adh;
        result[2] = +((C_sod - 140 + eps_aum - 0.2*0.2787*Math.exp(Fi_co*0.2281) - 7.0E-4*x[7])/3 - x[2])/T_adh;

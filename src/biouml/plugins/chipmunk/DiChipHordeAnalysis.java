...
import ru.autosome.di.ChIPHorde;
...
public class DiChipHordeAnalysis extends DiChipMunkAnalysis
{
    public DiChipHordeAnalysis(DataCollection origin, String name)
    {
        super(origin, name, new DiChipHordeAnalysisParameters());
    }
    ...
    public void setParameters(AnalysisParameters params) throws IllegalArgumentException
    {
        try
        {
            parameters = (DiChipHordeAnalysisParameters)params;
        }
        catch( Exception ex )
        {
    ...
    @Override
    public DiChipHordeAnalysisParameters getParameters()
    {
        return (DiChipHordeAnalysisParameters)parameters;
    }
    ...
    public FrequencyMatrix[] justAnalyzeAndPut() throws Exception
    {
        ...
        {
            mat++;
            FrequencyMatrix weightMatrix = new FrequencyMatrix(libraryDE, getParameters().getMatrixName() + "_" + mat, DiNucleotideAlphabet.getInstance(), null, transposeMatrix(result.getWPCM().getMatrix()), false);

...
public class SimulationEngineWrapper extends Option
{
    ...
    public void setSimulator(String solverName)
    {
        if( engine == null || solverName.isEmpty() )
            return;
        String oldValue = engine.getSolverName();
        engine.setSolverName( solverName );
        firePropertyChange( "simulator", oldValue, solverName );
        firePropertyChange( "*", null, null );
    }
    ...
    {
        if( engine != null )
            return engine.getSimulatorOptions();
        return null;
    }
    public void setSimulatorOptions(Options options)
    {
        if( engine != null )
            engine.setSimulatorOptions( options );
    }

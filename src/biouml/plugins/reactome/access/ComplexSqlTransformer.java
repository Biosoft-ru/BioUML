...
public class ComplexSqlTransformer extends PhysicalEntitySqlTransformer<Complex>
{
    @Override
    ...
    public Complex create(ResultSet resultSet, Connection connection) throws Exception
    {
        ...
        dps.add(new SqlDynamicProperty(ORGANISM_PD, getConnectionHolder(), getEntitySpeciesQuery( getReactomeId(complex), "Complex_2_species"), true));
    ...
    public String getNameListQuery()
    {
        ...
        + " dbt ON(dbt.stableIdentifier=si.DB_ID) WHERE dbt._class='Complex' ORDER BY " + idField;
    }
    ...
    public String getCountQuery()
    {
        ...
        + " dbt ON(dbt.stableIdentifier=si.DB_ID) WHERE dbt._class='Complex'";
    }
    ...
    protected String[] getComponents(@Nonnull Complex parent, Connection connection)
    {
        String memberQuery = "SELECT DISTINCT identifier, _displayName,_class FROM " + databaseObjectTable
                + " dbt INNER JOIN Complex_2_hasComponent es ON(es.hasComponent=dbt.DB_ID) JOIN StableIdentifier si ON (si.DB_ID=dbt.stableIdentifier) WHERE es.DB_ID='" + getReactomeId(parent)
                + "' ORDER BY hasComponent_rank";

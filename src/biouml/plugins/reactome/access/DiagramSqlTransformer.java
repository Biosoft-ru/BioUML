...
public class DiagramSqlTransformer extends SqlTransformerSupport<Diagram>
{
    ...
    protected static final Logger log = Logger.getLogger( DiagramSqlTransformer.class.getName() );

    public DiagramSqlTransformer()
    {
        super();
        idField = "DB_ID";
    ...
    public String getElementQuery(String name)
    {
        ...
        return "select p2r.db_id,p2r.representedPathway,do._displayName from PathwayDiagram_2_representedPathway p2r"
                + " join DatabaseObject do on p2r.representedPathway=do.DB_ID and representedPathway_rank=0 AND p2r.DB_ID=" + id;
    }

    @Override
    public String getCountQuery()
    {
        return "SELECT COUNT(1) FROM PathwayDiagram p";
    }

    @Override
    public String getNameListQuery()
    {
        return "SELECT p.DB_ID FROM PathwayDiagram p";
    }
    ...
    public String getElementExistsQuery(String name)
    {
        ...
        return "SELECT p.DB_ID FROM PathwayDiagram p WHERE p.DB_ID=" + id;
    }
    ...
    {
        return "select p2r.db_id,p2r.representedPathway,do._displayName from PathwayDiagram_2_representedPathway p2r"
                + " join DatabaseObject do on p2r.representedPathway=do.DB_ID and representedPathway_rank=0";
    }
    ...
    protected Reaction getReaction(String reactionID, Connection connection, String collectionName)
    {
        try
        {
            ...
            {
                return Module.getModulePath(owner).getChildPath(Module.DATA, collectionName, reactionName).getDataElement(Reaction.class);
            }
        }
        catch( Exception e )
        {
            ...
            if( specie instanceof Base )
            {
                ...
                else
                {
                    ...
                    if( "SimpleEntity".equals( ( (Base)specie ).getAttributes().getValue("Class")) )
                    {
                        ...
                        specieNode.getAttributes().add(
                                new DynamicProperty(SBGNPropertyConstants.SBGN_ENTITY_TYPE_PD, String.class, "unspecified"));
                    }

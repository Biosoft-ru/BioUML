...
public class GillespieSolver extends StochasticSimulator
{
    ...
    public boolean doStep()
    {
        try
        {
            ...
            double nextReactionTime = ( timeBeforeNextReaction == Double.POSITIVE_INFINITY ) ? timeLimit : time + timeBeforeNextReaction;
            ...
            if( modelHasEvents )
            {
                int flag = checkEvents(nextReactionTime, timeLimit);
                switch( flag )
                {
                    case EVENT_OCCURED:
                        return false;
                    case SPAN_POINT_BEFORE_EVENT:
...
//    protected void removeSpanPoint(int index)
//    {
//        double[] oldArray = span.getTimes();
//        double[] newArray = new double[span.getLength() - 1];
//        System.arraycopy(oldArray, 0, newArray, 0, index);
//        System.arraycopy(oldArray, index + 1, newArray, index, oldArray.length - index - 1);
//        Span = new ArraySpan(newArray);
//    }

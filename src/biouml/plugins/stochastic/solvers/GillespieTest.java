...
public class GillespieTest extends StochasticSimulator
{
    ...
    public boolean doStep()
    {
        try
        {
            ...
            while( nextSpanIndex < span.getLength() )
            {
                ...
                while( nextSpanIndex < span.getLength() && span.getTime(nextSpanIndex) <= nextReactionTime )
                {
                    //                if( debug )
                    //                    System.out.println("Out time = " + span.getTime(currentSpanIndex + 1) + " x[1]= " + xOut[1]);
                    time  = span.getTime(nextSpanIndex);
                    fireSolutionUpdate(time, x);
                    nextSpanIndex++;

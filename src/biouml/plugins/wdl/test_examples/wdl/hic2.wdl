...
workflow hic {
    ...
    scatter(i in range(length(fastq))) {
        ...
        call merge { input:
            bams = flatten(select_all([chimeric_sam_specific.output_bam,chimeric_sam_nonspecific.output_bam])),
        ...
        call calculate_stats as calculate_stats_on_library { input:
            alignment_stats = chimeric_sam_specific.stats,
            bam = dedup.deduped_bam,
    ...
    scatter(i in range(length(qualities))) {
        ...
        call calculate_stats { input:
            ...
            ligation_site = select_first([ligation_site]),
            quality = qualities[i],
            temp_anna_1 = align.bam_and_ligation_count[0][0],
            single_ended = temp_anna_1.single_ended,
            runtime_environment = runtime_environment,
        }
        ...
        File unnormalized_hic_file = select_first([create_hic.output_hic, create_hic_with_chrom_sizes.output_hic])

...
public class CRAMFileTransformer extends AbstractFileTransformer<BAMTrack> implements PriorityTransformer
{
    ...
    public FileDataElement transformOutput(BAMTrack output) throws Exception
    {
        ...
        try
        {
            ...
            File indexFile = output.getIndexFile();
            if(indexFile.exists()) {
                File bai_output = BAMTrack.getIndexFile( fde.getFile() );
                ApplicationUtils.linkOrCopyFile(bai_output, indexFile, null);
            }
            return fde;
        }
        finally

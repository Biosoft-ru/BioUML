...
def double_encode( fastq, out_dir ):
  de_fastq = tempfile.NamedTemporaryFile(dir=out_dir).name
  out = open( de_fastq, 'wb' )
  table = maketrans("0123.", "ACGTN")
  n = 0
  for line in file( fastq, 'rb'):
    line = line.rstrip("\r\n")
    if n == 1:
      line = line[1:].translate(table)
    if n == 3:
      line = line[1:]
    out.write(line + "\n")
    n = (n + 1) % 4
...
def __main__():
    ...
    executable="bwa"
    if options.color_space: executable = "/opt/galaxy/tools/bwa/bwa-0.5.9/bwa"
    # output version # of tool
    try:
        tmp = tempfile.NamedTemporaryFile().name
        tmp_stdout = open( tmp, 'wb' )
        proc = subprocess.Popen( args='%s 2>&1' % (executable), shell=True, stdout=tmp_stdout )
        tmp_stdout.close()
    ...
    if options.color_space:
       color_space = '-c'
       fastq = double_encode(fastq, tmp_dir)
       if options.genAlignType == 'paired':
           rfastq = double_encode(rfastq, tmp_dir)
    ...
    if options.fileSource == 'history' and not options.do_not_build_index:
        ...
        cmd1 = '%s index %s %s' % ( executable, indexing_cmds, ref_file_name )
        try:
    ...
    cmd2 = '%s aln %s %s %s > %s' % ( executable, aligning_cmds, ref_file_name, fastq, tmp_align_out_name )
    cmd2b = ''
    if options.genAlignType == 'paired':
        cmd2b = '%s aln %s %s %s > %s' % ( executable, aligning_cmds, ref_file_name, rfastq, tmp_align_out2_name )
        cmd3 = '%s sampe %s %s %s %s %s %s | samtools view -Su - | samtools sort -o - - > %s' % ( executable, gen_alignment_cmds, ref_file_name, tmp_align_out_name, tmp_align_out2_name, fastq, rfastq, options.output )
    else:
        cmd3 = '%s samse %s %s %s %s' % ( executable, gen_alignment_cmds, ref_file_name, tmp_align_out_name, fastq )
        if options.color_space:
          cmd3 = '%s | add_CSCQ.py %s' % ( cmd3, options.fastq )
        cmd3 = '%s | samtools view -Su - | samtools sort -o - - > %s' % (cmd3, options.output)
    # perform alignments
    buffsize = 1048576
    try:

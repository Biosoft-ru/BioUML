...
public class TextUtilTest extends TestCase
{
    public void testInsertBreaks()
    {
        assertEquals("xxxxxxxxxxx,xxxxxxxx,xxxxxxxxx,xxxxxxxxx,xxxxxxxxxx, xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxxxx",
                TextUtil.insertBreaks( "xxxxxxxxxxx,xxxxxxxx,xxxxxxxxx,xxxxxxxxx,xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxxxx" ));
        assertEquals("xxxxxxxxxxx,xxxxxxxx,xxxxxxxxx,xxxxxxxxx,xxxxxxxxxx, "
                ...
                TextUtil.insertBreaks( "xxxxxxxxxxx,xxxxxxxx,xxxxxxxxx,xxxxxxxxx,xxxxxxxxxx,"
                        + "xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxxxxxxxxxxxxx,xxxxxxxxx,"
                        + "xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxxxxxxxxxxxxx,"
                        + "xxxxxxxxx,xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxxxx" ));
        assertEquals("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxA Bxxxxxxxxxxxxxxxxxx ddd",
                TextUtil.insertBreaks( "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxABxxxxxxxxxxxxxxxxxx ddd" ));
        assertEquals("Axxxxxxxxxx,xxxxxxxx,xxxxxxxxx,xxxxxxxxx,xxxxxxxxxx, "
                ...
                TextUtil.insertBreaks( "Axxxxxxxxxx,xxxxxxxx,xxxxxxxxx,xxxxxxxxx,xxxxxxxxxx,"
                        + "Axxxxxxxxx,xxxxxxxxxx,xxxxxxxxxxxxxxxxxxxxx,xxxxxxxxx,"
                        + "AxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxB"
                        + "Axxx,xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxx,xxxxxxxxxxxx" ));
    ...
    {
        assertNull(TextUtil.toLower(null));
        assertEquals("CMA", TextUtil.toLower("CMA"));
        assertEquals("some text", TextUtil.toLower("Some text"));
        assertEquals("use HTTP", TextUtil.toLower("Use HTTP"));
    }

    public void testWildcardToRegex() throws Exception
    {
        assertNull(TextUtil.wildcardToRegex(null));

        Pattern pattern = Pattern.compile(TextUtil.wildcardToRegex("*.*"));
        ...
        pattern = Pattern.compile(TextUtil.wildcardToRegex("test"));
        assertFalse(pattern.matcher("test.").matches());
        assertTrue(pattern.matcher("test").matches());
        ...
        pattern = Pattern.compile(TextUtil.wildcardToRegex("????.???"));
    ...
    {
        Bean bean = new Bean();
        String text = TextUtil.toString( bean );
        assertEquals("{\"field\":null}", text);
        Bean bean2 = (Bean)TextUtil.fromString( Bean.class, text );
        assertNotNull(bean2);
    ...
    public void testArrayWithNull()
    {
        ...
        String text = TextUtil.toString( array );
        assertEquals( "[\"a\",null,\"b\"]", text );
        String[] array2 = (String[])TextUtil.fromString( String[].class, text );
